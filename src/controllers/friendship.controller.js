...
exports.sendFriendRequest = async (req, res) => {
  ...
  if (req.user._id == to) {
    return res.status(400).json({
      status: 400, message: 'You cannot send a friend request to yourself',
    });
  }

  // check if already friends
  if (to in req.user.friends) {
    return res.status(400).json({
      status: 400, message: 'You already friends',
    });
  }
...
exports.getFriendRequests = async (req, res) => {
  ...
  const requests = await friendshipService
    .getFriendRequests(user._id, limit, offset);
